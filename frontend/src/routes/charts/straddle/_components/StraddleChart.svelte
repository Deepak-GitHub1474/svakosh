<script lang="ts">
	...
	let { data, symbol, strike, chartType = $bindable('line') }: Props = $props();
	...
	function updateOptions() {
		...
		const options = getStraddleChartOptions(data, colors, chartType);
...
{:else}
	<SvaKoshCard 
		class="transition-all duration-500 ease-in-out hover:border-primary/20 flex flex-col pt-3 relative h-[590px]"
		meta={title}
	>
		<div 
			bind:this={chartContainer} 
	...
	<div class="flex flex-col sm:flex-row sm:items-center justify-between w-full gap-3 -mt-1 sm:mt-0 pb-2 sm:pb-0">
		<div class="flex items-center gap-3">
			<h4 class="text-[0.625rem] text-muted-foreground uppercase tracking-[0.2em] flex items-center gap-2">
				...
				<span class="text-foreground font-normal whitespace-nowrap">{symbol} Straddle Analysis</span> 
