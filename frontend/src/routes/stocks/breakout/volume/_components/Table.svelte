<script lang="ts">
	import type { TVolumeBreakout, TSortConfig, TSortKey } from '../_lib/types';
	import { formatNumber } from '$lib/utils';
	import SearchOff from '$lib/components/svg-provider/SearchOff.svelte';
	import { VOLUME_BREAKOUT_COLUMNS } from '../_lib/const';

	interface Props {
		breakouts: TVolumeBreakout[];
		...
		<table class="w-full text-left border-separate border-spacing-0 select-none">
			<thead class="sticky top-0 z-20">
				<tr class="bg-surface text-muted-foreground border-b border-white/5">
					{#each VOLUME_BREAKOUT_COLUMNS as col}
						<th
			...
			<tbody>
					...
					<tr class="hover:bg-white/5 transition-colors group">
						{#each VOLUME_BREAKOUT_COLUMNS as col}
							...
							>
								...
								{:else if col.key === 'change_wrt_high' || col.key === 'change_wrt_low'}
									<span class="font-medium {item[col.key] >= 0 ? 'text-bullish' : 'text-bearish'}">
										{item[col.key] >= 0 ? '+' : ''}{item[col.key].toFixed(2)}%
									</span>
				...
				{:else}
					<tr>
						<td colspan={VOLUME_BREAKOUT_COLUMNS.length} class="px-6 py-32 text-center">
							<div
