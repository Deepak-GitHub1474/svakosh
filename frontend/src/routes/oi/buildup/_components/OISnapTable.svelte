...
<div class="glass-panel overflow-hidden rounded-xl border border-white/5 flex flex-col bg-[#14171a]/40 backdrop-blur-sm">
	<div class="flex border-b border-white/10 bg-[#1c1f24]">
		<div class="flex-1 text-center py-2 text-xs text-muted-foreground uppercase border-r border-white/5">CE</div>
		<div class="flex-[1.5] text-center py-2 text-sm text-primary tracking-wider">{symbol}</div>
		<div class="flex-1 text-center py-2 text-xs text-muted-foreground uppercase border-l border-white/5">PE</div>
	</div>

	<div class="grid grid-cols-4 text-[0.65rem] text-muted-foreground bg-white/5 border-b border-white/5">
		<div class="py-2 text-center border-r border-white/5">Strike / CW</div>
		<div class="py-2 text-center border-r border-white/5">Strike / CU</div>
		<div class="py-2 text-center border-r border-white/5">Strike / PW</div>
	...
	<div class="grid grid-cols-4 text-[0.7rem]">
		<div class="border-r border-white/5 flex flex-col">
			{#each data.CE.Writing as item, i}
				<div class="flex justify-between px-2 py-1.5 border-b border-white/5 {i % 2 === 0 ? 'bg-white/[0.02]' : ''}">
					<span class="text-foreground/60">{item.Strike || '-'}</span>
					<span class={getPriceClass(item.OI)}>{formatNumber(item.OI) || '0.00'}</span>
				</div>
			{/each}
			<div class="mt-auto bg-bullish-subtle/30 py-2 border-t border-white/10 flex justify-between px-2 items-center text-bullish">
		...
		<div class="border-r border-white/5 flex flex-col">
			...
			{/each}
			<div class="mt-auto bg-bearish-subtle/30 py-2 border-t border-white/10 flex justify-between px-2 items-center text-bearish">
