...
export function getTrackerChartOptions(timestamps: string[], data: any, oiOption: string, combined: boolean, existingSelection?: Record<string, boolean>) {
	...
	if (combined) {
		['CE', 'PE'].forEach(side => {
			categories.forEach(cat => {
				...
				series.push({
					...
					lineStyle: { width: 0.8, opacity: 0.7 },
					itemStyle: { color }
				});
			});
		});
		...
		categories.forEach((cat, idx) => {
			...
			series.push({
				...
				lineStyle: { 
					width: 1.2, 
					color: Object.values(colors)[idx % 5],
					opacity: 0.7 
				}
			});
		});
