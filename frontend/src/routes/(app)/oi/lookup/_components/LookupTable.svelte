...
<div class="flex flex-col xl:flex-row gap-4 mb-6">
			...
			<table class="w-full text-xs border-collapse">
				<thead>
					<tr class="bg-[#1c1f24] text-muted-foreground border-b border-border-subtle">
						<th class="py-2 px-3 border-r border-border-subtle">Category</th>
				...
				<tbody>
					{#each LOOKUP_CATEGORIES as cat}
						<tr class="border-b border-border-subtle hover:bg-white/[0.02] transition-colors group">
							...
							{#each LOOKUP_TIMEFRAMES as tf}
								<td class={cn('px-2 py-2 text-center border-r border-border-subtle', getValClass(data.CE[cat][tf]))}>
	...
	<div class="hidden xl:flex flex-col bg-[#1c1f24] border border-border-subtle rounded-xl divide-y divide-white/10 overflow-hidden">
		<div class="py-2 px-3 text-xs text-muted-foreground text-center uppercase border-b border-border-subtle tracking-widest">PCR</div>
		<div class="py-2 px-3 text-xs text-muted-foreground text-center border-b border-border-subtle bg-[#1c1f24]/80 tracking-tighter">Ratio</div>
				...
				<tbody>
					{#each LOOKUP_CATEGORIES as cat}
						<tr class="border-b border-border-subtle hover:bg-white/[0.02] transition-colors group">
							...
							{#each LOOKUP_TIMEFRAMES as tf}
								<td class={cn('px-2 py-2 text-center border-r border-border-subtle', getValClass(data.PE[cat][tf]))}>
